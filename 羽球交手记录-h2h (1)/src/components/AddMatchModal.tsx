...
import { X, Trophy, Calendar, MapPin, ChevronRight, Plus, Minus } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="grid grid-cols-[1fr,40px,1fr] gap-4 items-center">
            <div className="space-y-2">
              ...
              <button 
                ...
                className="w-full min-h-[60px] bg-white rounded-2xl border border-neutral-100 flex flex-col items-center justify-center p-2 text-sm font-bold shadow-sm"
              >
            ...
            <div className="flex justify-center pt-6">
              <div className="text-xs font-black text-neutral-200">VS</div>
            </div>
          ...
          <div className="space-y-4">
             ...
             <div className="space-y-3">
               {scores.map((score, idx) => (
                 <div key={idx} className="flex items-center gap-4 bg-white p-3 rounded-2xl border border-neutral-100 shadow-sm">
                   ...
                   <div className="flex items-center gap-2 flex-1 justify-center">
                      ...
                      <input 
                        ...
                        className="w-12 text-center text-xl font-black outline-none"
                      />
               ...
               <button 
                ...
                className="w-full py-3 border-2 border-dashed border-neutral-200 rounded-2xl text-neutral-400 text-sm font-bold flex items-center justify-center gap-2 hover:bg-neutral-100 transition-colors"
               >
                 <Plus size={16} /> 添加更多局数
               </button>
             </div>
        ...
        {/* Nested Player Selection */}
        {isChoosingPlayers && (
          <NestedPlayerSelect 
            side={isChoosingPlayers} 
            players={players} 
            selected={isChoosingPlayers === 'team1' ? team1 : team2}
            onSelect={(ids) => {
              if (isChoosingPlayers === 'team1') setTeam1(ids);
              else setTeam2(ids);
              setIsChoosingPlayers(null);
            }}
            onClose={() => setIsChoosingPlayers(null)}
          />
        )}
      </motion.div>
    </motion.div>
...
function NestedPlayerSelect({ side, players, selected: initialSelected, onSelect, onClose }: any) {
  ...
  return (
    <div className="absolute inset-0 bg-white z-[80] flex flex-col">
       <div className="bg-red-600 p-6 text-white flex items-center justify-between shadow-lg">
          ...
          <span className="font-bold">选择 {side} 球员</span>
          <button onClick={() => onSelect(selected)} className="font-bold">确定</button>
       </div>
       <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {players.map((p: any) => (
            <button 
              key={p.id}
              onClick={() => toggle(p.id)}
              className={`w-full flex items-center justify-between p-4 rounded-xl border ${selected.includes(p.id) ? 'bg-red-50 border-red-200' : 'bg-neutral-50 border-transparent'}`}
            >
              ...
              {selected.includes(p.id) && <ChevronRight className="text-red-500" />}
            </button>
          ))}
       </div>
    </div>
