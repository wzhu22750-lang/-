...
import { X, Trophy, Target, Award, ChevronRight, Users, Flame, Star, Activity, Zap } from 'lucide-react';
...
export function PlayerProfileModal({ player, matches, players, onClose }: PlayerProfileModalProps) {
  ...
  const getWeeklyRank = (type: 'Singles' | 'Doubles') => {
    ...
    const rank = weeklyData.findIndex(x => x.id === player.id) + 1;
    const isFirst = rank === 1 && weeklyData[0].wins > 0;
    return { rank: weeklyData[rank-1]?.wins > 0 ? `#${rank}` : '--', isFirst };
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[150] flex items-center justify-center overflow-y-auto">
      <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="bg-[#f8f9fa] w-full max-w-lg min-h-screen sm:min-h-0 sm:rounded-[48px] overflow-hidden relative shadow-2xl flex flex-col">
          ...
          <div className="flex justify-between items-center relative z-10">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <h1 className="text-4xl font-black text-white italic tracking-tighter">{player.name}</h1>
                {(sRank.isFirst || dRank.isFirst) && (
                  <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="bg-yellow-400 text-black px-2 py-0.5 rounded text-[8px] font-black uppercase">Weekly Champ</motion.div>
                )}
              </div>
              <div className="flex gap-4">
                 <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                    <p className="text-white/30 text-[8px] font-black uppercase tracking-widest mb-1">Club ELO</p>
                    <p className="text-xl font-black text-red-500 italic">{player.elo_rating}</p>
                 </div>
                 <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                    <p className="text-white/30 text-[8px] font-black uppercase tracking-widest mb-1">Status</p>
                    <p className="text-sm font-bold text-green-400 uppercase tracking-tighter flex items-center gap-1"><Zap size={10} fill="currentColor"/> Active</p>
                 </div>
              </div>
            </div>

            <div className="relative">
              <div className="w-28 h-28 rounded-[32px] p-1 bg-gradient-to-tr from-red-600 to-red-400 rotate-3">
                <div className="w-full h-full rounded-[28px] border-4 border-[#222] overflow-hidden bg-neutral-800 -rotate-3">
                  {player.avatar ? <img src={player.avatar} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-3xl font-black text-white/20 italic">{player.initials}</div>}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="px-6 -mt-8 relative z-20 flex-1 overflow-y-auto no-scrollbar pb-10">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
               <StatBox label="Weekly Wins" value={weekStats.sWins + weekStats.dWins} sub="This week total" color="text-red-500" icon={<Activity size={20} className="text-red-500" />} />
               <StatBox label="Win Rate" value={`${winRate}%`} sub="All time average" color="text-blue-500" icon={<Star size={20} className="text-blue-500" fill="currentColor"/>} />
            </div>
            ...
            <div className="space-y-4">
               <div className="flex items-center justify-between px-2">
                  <h3 className="text-sm font-black text-neutral-800 uppercase italic">Major Opponents</h3>
                  <span className="text-[9px] font-bold text-neutral-300 tracking-tighter">TOTAL MATCHED</span>
               </div>
               <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
                  {topOpponentIds.map(([oid, count]) => {
        ...
        <div className="p-8 bg-white border-t border-neutral-50">
           <button onClick={onClose} className="w-full py-4 bg-[#111] text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-xl active:scale-95 transition-all">Dismiss Dossier</button>
        </div>
      </motion.div>
    </motion.div>
  );
...
function StatBox({ label, value, sub, icon }: any) {
  return (
    <div className="bg-white p-5 rounded-[32px] shadow-sm border border-neutral-50 flex flex-col justify-between h-36">
       <div className="w-full flex justify-between items-start">{icon}</div>
       <div>
          <p className="text-2xl font-black text-neutral-900 italic leading-none">{value}</p>
          <p className="text-[9px] font-black text-neutral-400 uppercase mt-2 tracking-tighter">{label}</p>
          <p className="text-[8px] font-bold text-neutral-300 mt-0.5">{sub}</p>
       </div>
    </div>
  );
}
